...
const CATEGORIES = ['Fuel System', 'Air Filter', 'Oil Filter', 'Hydraulic', 'Transmission', 'DEF', 'Coolant', 'Other'];
...
function CustomerForm({
  ...
  const [form, setForm] = useState<Partial<Customer>>(
    initial || { name: '', contact_name: '', phone: '', email: '', billing_address: '', notes: '' }
  );
  return (
    <div className="mbg" onClick={onClose}>
      <div className="mdl" onClick={e => e.stopPropagation()}>
        <div className="mh">
          <h3>{initial?.customer_id ? 'Edit Customer' : 'Add Customer'}</h3>
          <button className="btn bs bg" onClick={onClose}>✕</button>
        </div>
        <div className="mb2">
          <div className="fld">
        ...
        <div className="mf">
          ...
          <button className="btn bp" onClick={() => onSave(form)} disabled={!form.name}>Save</button>
        </div>
      </div>
    </div>
  );
...
function UnitForm({
  ...
  initial: Partial<FleetUnit> | null;
  customer_id?: string;
  engines: Engine[];
  onSave: (unit: Partial<FleetUnit>) => void;
  onClose: () => void;
}) {
  const [form, setForm] = useState<Partial<FleetUnit>>(
    initial || {
      customer_id: customer_id || '',
      unit_number: '',
      nickname: '',
      engine_id: engines[0]?.engine_id || null,
      type: '',
      make: '',
      model: '',
      year: '',
  ...
  return (
    <div className="mbg" onClick={onClose}>
      <div className="mdl" onClick={e => e.stopPropagation()}>
        <div className="mh">
          <h3>{initial?.unit_id ? 'Edit Unit' : 'Add Fleet Unit'}</h3>
          <button className="btn bs bg" onClick={onClose}>✕</button>
        </div>
        <div className="mb2">
          <div className="fr">
            <div className="fld"><label>Unit Number</label><input value={form.unit_number || ''} onChange={e => setForm(p => ({ ...p, unit_number: e.target.value }))} placeholder="e.g. I-14" /></div>
        ...
        <div className="mf">
          <button className="btn" onClick={onClose}>Cancel</button>
          <button className="btn bp" onClick={() => onSave(form)} disabled={!form.unit_number}>Save</button>
        </div>
      </div>
    </div>
  );
...
function EngineForm({
  ...
  initial: Partial<Engine> | null;
  onSave: (engine: Partial<Engine>) => void;
  onClose: () => void;
}) {
  const [form, setForm] = useState<Partial<Engine>>(
    initial || { name: '', manufacturer: '', displacement: '', oil_type: '15W-40', oil_capacity_gallons: 0, notes: '' }
  );
  return (
    <div className="mbg" onClick={onClose}>
      <div className="mdl" onClick={e => e.stopPropagation()}>
        <div className="mh">
          <h3>{initial?.engine_id ? 'Edit Engine' : 'Add Engine'}</h3>
          <button className="btn bs bg" onClick={onClose}>✕</button>
        </div>
        <div className="mb2">
          <div className="fld"><label>Engine Name</label><input value={form.name || ''} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Cat 4.4" /></div>
          <div className="fr">
...
function PartForm({
  ...
  const [form, setForm] = useState<Partial<Part>>(
    initial || { part_number: '', description: '', manufacturer: 'CAT', category: 'Other', retail_price: 0, cost: 0, supplier: 'CAT', notes: '' }
  );
  ...
  return (
    <div className="mbg" onClick={onClose}>
      <div className="mdl" onClick={e => e.stopPropagation()}>
        <div className="mh">
          <h3>{initial?.part_id ? 'Edit Part' : 'Add Part'}</h3>
          <button className="btn bs bg" onClick={onClose}>✕</button>
        </div>
        <div className="mb2">
          <div className="fr">
            <div className="fld"><label>Part Number</label><input value={form.part_number || ''} onChange={e => setForm(p => ({ ...p, part_number: e.target.value }))} /></div>
            <div className="fld">
        ...
        <div className="mf">
          <button className="btn" onClick={onClose}>Cancel</button>
          <button className="btn bp" onClick={() => onSave(form)} disabled={!form.part_number || !form.description}>Save</button>
        </div>
      </div>
    </div>
  );
...
function SettingsForm({
  ...
  onSave: (s: Partial<CompanySettings>) => void;
}) {
  ...
  return (
    <div className="card">
      ...
      <p style={{ fontSize: 12, color: 'var(--tx2)', marginBottom: 14 }}>
        Your business info. This will appear on invoices and work orders.
      </p>
      ...
      <button className="btn bp" onClick={() => onSave(form)}>Save Settings</button>
    </div>
  );
}
...
type ModalType =
  ...
  | { t: 'noSpec'; tier: string; engineName?: string };
...
export default function OilTrackApp({ user }: { user: User }) {
  ...
  const { data: specs = [] } = useEngineServiceSpecs();
  ...
  const getSpec = (engine_id: string, tier: string) => specs.find(s => s.engine_id === engine_id && s.tier === tier);
  const getCustomer = (id: string) => customers.find(c => c.customer_id === id);
  const getUnit = (id: string) => units.find(u => u.unit_id === id);

  async function buildWorkOrder(unit_id: string, tier: string): Promise<string | null> {
    const u = getUnit(unit_id);
    if (!u) return null;
    const customer = getCustomer(u.customer_id);
    if (!customer) return null;
    const engine = getEngine(u.engine_id);
    const spec = engine ? getSpec(engine.engine_id, tier) : null;

    const checklist: ChecklistItem[] = [];
    if (engine) checklist.push({ text: `Oil change — ${engine.oil_capacity_gallons || '?'} gal ${engine.oil_type || '15W-40'}`, done: false });
    if (spec?.parts_list) {
      for (const ref of spec.parts_list) {
        const p = parts.find(x => x.part_number === ref.part_number);
        checklist.push({ text: `${p?.description || ref.part_number} replaced (${ref.part_number})`, done: false });
      }
    }
    checklist.push(
      { text: 'Greased all lube points', done: false },
      { text: 'Oil sample collected', done: false },
      { text: 'Checked for leaks', done: false },
      { text: 'Recorded engine hours', done: false }
    );
    ...
    const oil = engine && engine.oil_capacity_gallons > 0 ? {
      type: engine.oil_type,
      gallons: engine.oil_capacity_gallons,
      retail_per_gallon: customer.default_fees.oil_retail_per_gallon,
      cost_per_gallon: customer.default_fees.oil_cost_per_gallon,
      total_retail: round2(engine.oil_capacity_gallons * customer.default_fees.oil_retail_per_gallon),
      total_cost: round2(engine.oil_capacity_gallons * customer.default_fees.oil_cost_per_gallon),
    } : null;

    const fees = {
      labor: spec?.labor_override ?? customer.default_fees.labor_rate,
      environmental_fee: customer.default_fees.environmental_fee,
      travel_charge: customer.default_fees.travel_charge,
      service_supplies: customer.default_fees.service_supplies,
      oil_sample_fee: customer.default_fees.oil_sample_fee,
      other: 0,
    };
    ...
    try {
      const wo = await createWorkOrder.mutateAsync({
        ...
        engine_hours: u.current_hours || 0,
        technician: '',
        customer_provided_filters: false,
        notes: '',
  ...
  const sendAi = useCallback(async () => {
    ...
    const ctx = JSON.stringify({
      ...
      specs: specs.map(s => ({ engine: engines.find(x => x.engine_id === s.engine_id)?.name, tier: s.tier, parts: s.parts_list })),
    ...
    return (
      <>
        ...
        <div className="card">
          ...
          {cu.length === 0 ? (
            <div className="empty"><p>No equipment yet</p></div>
          ) : (
            <table>
              <thead><tr><th>Unit #</th><th>Description</th><th>Engine</th><th>Hours</th><th>WOs</th></tr></thead>
              <tbody>
                {cu.map(u => (
    ...
    return (
      <>
        ...
        <div className="card">
          ...
          <p style={{ fontSize: 12, color: 'var(--tx2)', marginBottom: 8 }}>Select a service tier.</p>
          <div className="tg2">
            {TIERS.map(t => {
              const sp = e ? getSpec(e.engine_id, t) : null;
              return (
                <div
                  key={t}
                  className={`tb2${sp ? '' : ' off'}`}
                  onClick={async () => {
                    if (!e) {
                      alert('This unit has no engine assigned. Edit the unit to attach one.');
                      return;
                    }
                    if (!sp) {
                      setModal({ t: 'noSpec', tier: t, engineName: e.name });
                      return;
                    }
                    const wid = await buildWorkOrder(u.unit_id, t);
                    if (wid) nav('wo', wid, t);
                  }}
                >
                  <h4>{t}</h4>
                  <p>{sp ? `${sp.parts_list.length} parts` : 'No spec'}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="card">
          ...
          {uWOs.length === 0 ? (
            <div className="empty"><p>No work orders</p></div>
          ) : (
            <table>
  ...
  const renderWO = () => {
    ...
    const updField = (k: string, v: any) => {
      updateWorkOrder.mutate({ work_order_id: wo.work_order_id, updates: { [k]: v } });
    };
    const updFee = (k: string, v: string) => {
      ...
      const totals = calculateWorkOrderTotals({
        ...
        oil: wo.oil_used,
        fees: newFees,
        tax_rate_percent: wo.tax_rate_percent,
      });
    ...
    const complete = () => {
      updateWorkOrder.mutate({
        ...
        updates: { status: 'complete', completed_at: new Date().toISOString() },
      });
      ...
      <div className="wo">
        <div className="wh">
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 700, fontFamily: "'IBM Plex Mono',monospace", background: isOpen ? 'var(--am2)' : 'var(--gn2)', color: isOpen ? 'var(--am)' : 'var(--gn)' }}>
        ...
        <div className="ws">
          ...
          <div className="wi">
            ...
            <div>
              <label>Date</label>
              {isOpen ? <input type="date" value={wo.service_date} onChange={ev => updField('service_date', ev.target.value)} style={inp} /> : <p className="m">{wo.service_date}</p>}
            </div>
            <div>
              <label>Engine Hours</label>
              {isOpen ? <input type="number" value={wo.engine_hours || 0} onChange={ev => updField('engine_hours', Number(ev.target.value) || 0)} style={{ ...inp, fontFamily: "'IBM Plex Mono',monospace", width: 120 }} /> : <p className="m">{wo.engine_hours || '—'}</p>}
            </div>
            <div>
              <label>Technician</label>
              {isOpen ? <input value={wo.technician} onChange={ev => updField('technician', ev.target.value)} placeholder="Name" style={{ ...inp, width: 140 }} /> : <p>{wo.technician || '—'}</p>}
            </div>
            <div><label>Invoice #</label><p className="m" style={{ fontWeight: 700 }}>{formatInvoiceNumber(wo.invoice_number)}</p></div>
            {isOpen && (
              <div>
                <label>Cust. Filters</label>
        ...
        <div className="ws">
          ...
          {wo.checklist.map((it, i) => (
            <div key={i} className="wk">
              {isOpen ? (
                <div className={`ck${it.done ? ' on' : ''}`} onClick={() => togChecklist(i)}>
                  {it.done && IC(iChk, 14)}
                </div>
              ) : (
                <div style={{ width: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', color: it.done ? 'var(--gn)' : 'var(--tx3)', fontSize: 14 }}>
                  {it.done ? '\u2713' : '\u25CB'}
                </div>
              )}
          ...
          {isOpen ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
              {[
                ...
                ['Environmental', 'environmental_fee'],
                ['Oil Sample', 'oil_sample_fee'],
                ['Other', 'other'],
              ].map(([l, k]) => (
            ...
            <>
              {Object.entries(wo.fees).filter(([, v]) => v > 0).map(([k, v]) => (
        ...
        {isOpen && (
          <div className="ws" style={{ background: allDone ? 'var(--gn2)' : 'var(--bg)', textAlign: 'center', padding: 20 }}>
            ...
            {!allDone && <p style={{ fontSize: 11, color: 'var(--tx3)', marginTop: 6 }}>{wo.checklist.filter(x => !x.done).length} unchecked</p>}
          </div>
        )}
      </div>
    );
    ...
    return (
      <>
        <div className="card">
          ...
          {open.length === 0 ? (
            <div className="empty"><p>No open work orders</p></div>
          ) : (
            <table>
              <thead><tr><th>Invoice #</th><th>Unit</th><th>Customer</th><th>Service</th><th>Date</th></tr></thead>
              <tbody>
  ...
  const renderEngines = () => (
    <div className="card">
      ...
      {engines.length === 0 ? (
        <div className="empty"><p>No engines yet. Create engine templates (e.g. "Cat 4.4") then attach them to fleet units.</p></div>
      ) : (
        <table>
    ...
    if (modal.t === 'noSpec') {
      return (
        <div className="mbg" onClick={() => setModal(null)}>
          <div className="mdl" onClick={e => e.stopPropagation()}>
            <div className="mh"><h3>No Spec</h3><button className="btn bs bg" onClick={() => setModal(null)}>✕</button></div>
            <div className="mb2"><p style={{ fontSize: 13, color: 'var(--tx2)' }}>No {modal.tier} spec for <strong>{modal.engineName}</strong> yet. (Building service spec editor in next round.)</p></div>
            <div className="mf"><button className="btn" onClick={() => setModal(null)}>OK</button></div>
          </div>
        </div>
      );
    }
    if (modal.t === 'addCust' || modal.t === 'editCust') {
      return (
        <CustomerForm
          ...
          onSave={(form) => {
            if (modal.t === 'addCust') {
              createCustomer.mutate(form, { onSuccess: () => setModal(null), onError: (err: any) => alert('Error: ' + err.message) });
            } else {
              updateCustomer.mutate({ customer_id: (modal.d as Customer).customer_id, updates: form }, { onSuccess: () => setModal(null), onError: (err: any) => alert('Error: ' + err.message) });
            }
          }}
        />
      );
    ...
    if (modal.t === 'addUnit' || modal.t === 'editUnit') {
      return (
        <UnitForm
          ...
          onSave={(form) => {
            if (modal.t === 'addUnit') {
              createUnit.mutate(form, { onSuccess: () => setModal(null), onError: (err: any) => alert('Error: ' + err.message) });
            } else {
              updateUnit.mutate({ unit_id: (modal.d as FleetUnit).unit_id, updates: form }, { onSuccess: () => setModal(null), onError: (err: any) => alert('Error: ' + err.message) });
            }
          }}
        />
      );
    ...
    if (modal.t === 'addEngine' || modal.t === 'editEngine') {
      return (
        <EngineForm
          ...
          onClose={() => setModal(null)}
          onSave={(form) => {
            if (modal.t === 'addEngine') {
              createEngine.mutate(form, { onSuccess: () => setModal(null), onError: (err: any) => alert('Error: ' + err.message) });
            } else {
              updateEngine.mutate({ engine_id: (modal.d as Engine).engine_id, updates: form }, { onSuccess: () => setModal(null), onError: (err: any) => alert('Error: ' + err.message) });
            }
          }}
        />
      );
    ...
    if (modal.t === 'addPart' || modal.t === 'editPart') {
      return (
        <PartForm
          ...
          onSave={(form) => {
            if (modal.t === 'addPart') {
              createPart.mutate(form, { onSuccess: () => setModal(null), onError: (err: any) => alert('Error: ' + err.message) });
            } else {
              updatePart.mutate({ part_id: (modal.d as Part).part_id, updates: form }, { onSuccess: () => setModal(null), onError: (err: any) => alert('Error: ' + err.message) });
            }
          }}
        />
      );
  ...
  const crumbs = trail.length > 0 && (
    <div className="crumbs">
      <span onClick={() => { setTrail([]); setPg(active); }}>
        {active === 'workorders' ? 'Work Orders' : 'Customers'}
      </span>
      {trail.map((n, i) => (
  ...
  const pages: Record<string, () => React.ReactNode> = {
    ...
    cust: renderCustDetail,
    unit: renderUnitDetail,
    wo: renderWO,
    workorders: renderWOs,
