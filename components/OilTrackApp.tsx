...
// =============================================
// FORMS — Customer, Unit, Engine, Part
// =============================================
function CustomerForm({ initial, onSaveAsync, onClose }: { initial: Partial<Customer> | null; onSaveAsync: (c: Partial<Customer>) => Promise<void>; onClose: () => void }) {
  const [form, setForm] = useState<Partial<Customer>>(initial || { name: '', contact_name: '', phone: '', email: '', billing_address: '', notes: '' });
  return (
    <ModalShell title={initial?.customer_id ? 'Edit Customer' : 'Add Customer'} onClose={onClose} onSave={() => onSaveAsync(form)} saveDisabled={!form.name}>
...
// =============================================
// SERVICE TEMPLATE EDITOR (the new big one)
// =============================================
function ServiceTemplateForm({
  ...
  initial: Partial<ServiceTemplate> | null;
  unit_id: string;
  parts: Part[];
  onSaveAsync: (t: Partial<ServiceTemplate>) => Promise<void>;
  onClose: () => void;
}) {
  ...
  const addPart = (part_number: string) => {
    setForm(prev => {
      ...
      if (existing) {
        return {
          ...prev,
          parts_list: prev.parts_list!.map(p =>
            p.part_number === part_number ? { ...p, qty: p.qty + 1 } : p
          ),
        };
      }
      return {
        ...prev,
        parts_list: [...(prev.parts_list || []), { part_number, qty: 1 }],
      };
    });
    setPartSearch('');
  };
  ...
  const removePart = (part_number: string) => {
    setForm(prev => ({
      ...prev,
      parts_list: prev.parts_list?.filter(p => p.part_number !== part_number) || [],
    }));
  };
  ...
  const setPartQty = (part_number: string, qty: number) => {
    setForm(prev => ({
      ...prev,
      parts_list: prev.parts_list?.map(p =>
        p.part_number === part_number ? { ...p, qty: Math.max(1, qty) } : p
      ) || [],
    }));
  };
  ...
  const setTask = (i: number, v: string) => {
    setForm(prev => ({
      ...prev,
      tasks: prev.tasks?.map((t, idx) => (idx === i ? v : t)) || [],
    }));
  };
      ...
      <div className="fr">
        <div className="fld">
          ...
          <input
            ...
            placeholder="e.g. 500hr Service, Wheel Gear Oil, Annual"
            autoFocus
          />
        </div>
        <div className="fld">
          ...
          <input
            ...
            step="0.01"
            value={form.labor || 0}
            onChange={e => setForm(p => ({ ...p, labor: Number(e.target.value) || 0 }))}
          />
        </div>
      </div>
            ...
            {form.parts_list.map(ref => {
              ...
              return (
                <div key={ref.part_number} className="tplist-item">
                  ...
                  <span className="desc">
                    {part ? `${part.description} — ${part.manufacturer}` : '⚠️ Part not in catalog'}
                  </span>
                  <div className="qty">
                    ...
                    <input
                      ...
                      min="1"
                      value={ref.qty}
                      onChange={e => setPartQty(ref.part_number, Number(e.target.value) || 1)}
                    />
                  </div>
        ...
        {partSearch.trim() && (
          <div className="psearchres">
            {filteredParts.length === 0 ? (
              <div className="row" style={{ color: 'var(--tx3)', cursor: 'default' }}>
                No parts match. Add new parts from the Parts Catalog.
              </div>
            ) : (
              filteredParts.map(p => (
          ...
          form.tasks.map((t, i) => (
            <div key={i} className="tklist-item">
              <input
                ...
                placeholder="e.g. Drain engine oil"
              />
...
function CopyTemplateForm({
  ...
  onSaveAsync: (template: Partial<ServiceTemplate>) => Promise<void>;
  onClose: () => void;
}) {
  ...
  const otherTemplates = useMemo(
    () => templates.filter(t => t.unit_id !== unit_id),
    [templates, unit_id]
  );

  const selected = otherTemplates.find(t => t.template_id === selectedId);

  // Prefill the new name with the original name when a template is picked
  useEffect(() => {
    if (selected && !newName) setNewName(selected.name);
  }, [selected]);
  ...
  return (
    <ModalShell
      ...
      onSave={async () => {
        ...
        await onSaveAsync({
          ...
          parts_list: selected.parts_list,
    ...
    >
      {otherTemplates.length === 0 ? (
        <div className="empty">
          <p>No templates exist on other units yet. Create one on another unit first, then copy from here.</p>
        </div>
      ) : (
        <>
          <div className="fld">
...
export default function OilTrackApp({ user }: { user: User }) {
  ...
  async function buildWorkOrderFromTemplate(template: ServiceTemplate): Promise<string | null> {
    ...
    // Checklist from template tasks (or default if empty)
    const checklist: ChecklistItem[] = (template.tasks.length > 0 ? template.tasks : [
      'Drain engine oil',
      'Replace filters',
      'Greased all lube points',
    ...
    // Oil from engine + customer pricing
    ...
    const fees = {
      labor: template.labor || customer.default_fees.labor_rate,
  ...
  // ─── Build a blank work order (one-off service) ───
  async function buildBlankWorkOrder(unit_id: string): Promise<string | null> {
    ...
    const fees = {
      labor: customer.default_fees.labor_rate,
      environmental_fee: customer.default_fees.environmental_fee,
      travel_charge: customer.default_fees.travel_charge,
      service_supplies: customer.default_fees.service_supplies,
      oil_sample_fee: customer.default_fees.oil_sample_fee,
  ...
  const sendAi = useCallback(async () => {
    ...
    const ctx = JSON.stringify({
      ...
      templates: templates.map(t => ({
        unit: getUnit(t.unit_id)?.unit_number,
        name: t.name,
        labor: t.labor,
        parts: t.parts_list.length,
        tasks: t.tasks.length,
      })),
      wos: workOrders.slice(0, 8).map(w => ({
        unit: getUnit(w.unit_id)?.unit_number,
        date: w.service_date,
        tier: w.tier,
        total: w.total_retail,
        status: w.status,
      })),
    });
    try {
      ...
      <>
                ...
                <div key={t.template_id} className="tpcard">
                  ...
                  <div className="row">
                    ...
                    <button
                      ...
                      onClick={() => setModal({
                        t: 'confirmDelete',
                        what: `the template "${t.name}"`,
                        onConfirm: async () => {
                          await deleteTemplate.mutateAsync(t.template_id);
        ...
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 8 }}>
            ...
            <button
              className="btn"
              style={{ padding: '14px 16px', fontSize: 13, justifyContent: 'flex-start', borderStyle: 'dashed' }}
              onClick={async () => {
  ...
  const renderEngines = () => (
    <div className="card">
      ...
      {engines.length === 0 ? <div className="empty"><p>No engines yet. Create engine templates (e.g. "Cat 4.4") then attach them to fleet units.</p></div> : (
        <table>
  ...
  const renderSettings = () => (
    <SettingsForm
      settings={company}
      onSaveAsync={async (s) => { await upsertCompany.mutateAsync(s); }}
    />
  );
