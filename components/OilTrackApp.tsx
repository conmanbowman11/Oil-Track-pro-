...
.mdl{background:var(--cd);border:1px solid var(--bd);border-radius:12px;width:100%;max-width:540px;max-height:85vh;overflow-y:auto;display:flex;flex-direction:column}
...
// =============================================
// MODAL SHELL — handles save/loading/error state for all forms
// IMPORTANT: clicking the backdrop does NOT close the modal anymore.
// Only X button, Cancel, or successful Save can close it.
// =============================================
...
function ModalShell({
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave();
      // Caller is expected to call onClose if successful.
    } catch (err: any) {
...
function CustomerForm({
  ...
  onSaveAsync: (customer: Partial<Customer>) => Promise<void>;
  onClose: () => void;
}) {
...
function UnitForm({
  ...
  customer_id?: string;
  engines: Engine[];
  onSaveAsync: (unit: Partial<FleetUnit>) => Promise<void>;
  onClose: () => void;
}) {
  ...
  return (
    <ModalShell
      title={initial?.unit_id ? 'Edit Unit' : 'Add Fleet Unit'}
      onClose={onClose}
      onSave={() => onSaveAsync(form)}
      saveDisabled={!form.unit_number}
    >
      <div className="fr">
...
function EngineForm({
  ...
  onSaveAsync: (engine: Partial<Engine>) => Promise<void>;
  onClose: () => void;
}) {
  ...
  return (
    <ModalShell
      title={initial?.engine_id ? 'Edit Engine' : 'Add Engine'}
      onClose={onClose}
      onSave={() => onSaveAsync(form)}
      saveDisabled={!form.name}
    >
...
function PartForm({
  ...
  onSaveAsync: (part: Partial<Part>) => Promise<void>;
  onClose: () => void;
}) {
  ...
  return (
    <ModalShell
      title={initial?.part_id ? 'Edit Part' : 'Add Part'}
      onClose={onClose}
      onSave={() => onSaveAsync(form)}
      saveDisabled={!form.part_number || !form.description}
    >
      <div className="fr">
...
// =============================================
// SETTINGS — inline form (not a modal). Properly loads existing data.
// =============================================
...
function SettingsForm({
  ...
  onSaveAsync: (s: Partial<CompanySettings>) => Promise<void>;
}) {
  ...
  // Load existing settings whenever they become available
  useEffect(() => {
...
export default function OilTrackApp({ user }: { user: User }) {
            ...
            {TIERS.map(t => {
              ...
              return (
                <div
                  ...
                  onClick={async () => {
                    if (!e) {
                      showToast('This unit has no engine assigned. Edit the unit to attach one.', true);
                      return;
                    }
                    if (!sp) {
  ...
  const renderSettings = () => (
    <SettingsForm
      settings={company}
      onSaveAsync={async (s) => {
        await upsertCompany.mutateAsync(s);
      }}
    />
  );
        ...
        <CustomerForm
          ...
          onSaveAsync={async (form) => {
            if (modal.t === 'addCust') {
              ...
              await updateCustomer.mutateAsync({ customer_id: (modal.d as Customer).customer_id, updates: form });
        ...
        <UnitForm
          ...
          onSaveAsync={async (form) => {
            if (modal.t === 'addUnit') {
              ...
              await updateUnit.mutateAsync({ unit_id: (modal.d as FleetUnit).unit_id, updates: form });
        ...
        <EngineForm
          ...
          onSaveAsync={async (form) => {
            if (modal.t === 'addEngine') {
              await createEngine.mutateAsync(form);
              showToast('Engine added');
            } else {
              await updateEngine.mutateAsync({ engine_id: (modal.d as Engine).engine_id, updates: form });
    ...
    if (modal.t === 'addPart' || modal.t === 'editPart') {
      return (
        <PartForm
          ...
          onSaveAsync={async (form) => {
            if (modal.t === 'addPart') {
              await createPart.mutateAsync(form);
              showToast('Part added');
            } else {
              await updatePart.mutateAsync({ part_id: (modal.d as Part).part_id, updates: form });
