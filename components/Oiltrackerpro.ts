...
export default function OilTrackApp({ user }: { user: User }) {
  ...
  async function buildWorkOrderFromTemplate(template: ServiceTemplate): Promise<string | null> {
    ...
    try {
      const wo = await createWorkOrder.mutateAsync({
        ...
        engine_hours: u.current_hours || 0, technician: '', customer_provided_filters: false, notes: '',
  ...
  const renderModal = () => {
    ...
    if (modal.t === 'addUnit' || modal.t === 'editUnit') {
      return (<UnitForm initial={modal.d || null} customer_id={modal.customer_id} engines={engines} onClose={() => setModal(null)} onSaveAsync={async (form) => {
